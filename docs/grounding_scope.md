...
`eval/abstract_diagnostic2.py`, `eval/mlm_loss_by_class.py`,
`eval/analyze_abstract.py` (no training required; run against the
...
**3. Benchmarks don't reward them.** Benchmark phenomena that hinge on
abstract and function words show no benefit from vision-init and a
...
validation MLM loss (`eval/mlm_loss_by_class.py`, identical masks across
models): vision-init predicts masked *function words* better than the
baseline in 3/3 seeds (mean −0.080 nats), with unseeded tokens as a
clean placebo (≈0). The retained structure is functionally load-bearing
for the training objective — the gap is in the *benchmarks*, which do
not reward what the model uses this information for.
...
Visual grounding helps a word on benchmarks only if its meaning is the
kind of thing vision can inform. For concrete nouns, the seeds encode
visual properties and the model benefits (see the VP-Swap results). For
...
"every" and "not" through all of training, and the training objective
itself measurably uses it (better mask prediction of function words in
3/3 seeds) — but no current benchmark rewards that use. The bottleneck
is what our evaluations measure, not seed quality and not retention.
...
words); the BLiMP gradient uses the fast subset; RSA cells use the main
seed pair at the final checkpoint; the MLM-loss test uses 3,000 held-out
validation lines with a single fixed masking.
